...
#Exercise 4.1
sol <- forwardDifference(f = function(x){-16*sin(8*pi*x)}, u0 = function(x){sin(pi*x)},
                         K = 0.25, T = 0.2)
library("plot3Drgl")

persp3D(sol$x, sol$t, sol$w, xlab = "x", ylab = "t", zlab = "Temp")
plotrgl()
...
#To evaluate the exact soln we construct the grid mesh
x <- numSol$x
t <- numSol$t
xy <- mesh(x, t)
...
max(abs(u - numSol$w)) # Maximal error
